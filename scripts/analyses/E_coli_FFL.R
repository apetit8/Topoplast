...
write.csv(rbindlist(plast_mg_c), "scripts/data/plast_mg_c_FFL.csv")
...
write.csv(rbindlist(plast_ox), "scripts/data/plast_ox_FFL.csv")
...
write.csv(rbindlist(plast_aero), "scripts/data/plast_aero_FFL.csv")
...
write.csv(rbindlist(plast_juice), "scripts/data/plast_juice_FFL.csv")
...
write.csv(rbindlist(plast_temptr), "scripts/data/plast_temptr_FFL.csv")
...
write.csv(rbindlist(plast_stringent), "scripts/data/plast_stringent_FFL.csv")
...
write.csv(rbindlist(plast_stress), "scripts/data/plast_stress_FFL.csv")
...
plast_FFL <- mclapply(plast_genes, function(gene) {
  gg <- induced.subgraph(g, vids = c(which(colnames(E_coli_mat)==gene), which(colnames(E_coli_mat)%in%TF_genes)) )
  E_coli_mat2 <- t((get.adjacency(gg, sparse=FALSE, attr='V3')))
  ggg <- induced.subgraph(gg, vids = as.vector(unlist(neighborhood(gg, cutoff.max-1, nodes = which(colnames(E_coli_mat2)==gene), mode = 'all'))))
  E_coli_mat3 <- t((get.adjacency(ggg, sparse=FALSE, attr='V3')))
...
write.csv(rbindlist(plast_FFL), "scripts/data/plast_genes_FFL.csv")
...
nonplast_FFL <- mclapply(nonplast_genes, function(gene) {
  gg <- induced.subgraph(g, vids = c(which(colnames(E_coli_mat)==gene), which(colnames(E_coli_mat)%in%TF_genes)) )
  E_coli_mat2 <- t((get.adjacency(gg, sparse=FALSE, attr='V3')))
  ggg <- induced.subgraph(gg, vids = as.vector(unlist(neighborhood(gg, cutoff.max-1, nodes = which(colnames(E_coli_mat2)==gene), mode = 'all'))))
  E_coli_mat3 <- t((get.adjacency(ggg, sparse=FALSE, attr='V3')))
...
write.csv(rbindlist(nonplast_FFL), "scripts/data/nonplast_FFL.csv")

print("Non plastic genes done!")
########################################
#Control : every genes
# 
# all_FFL <- mclapply(colnames(E_coli_mat), function(gene) {
#   gg <- induced.subgraph(g, vids = as.vector(unlist(neighborhood(g, cutoff.max-1, nodes = which(colnames(E_coli_mat)==gene), mode = 'all'))))
#   E_coli_mat2 <- t((get.adjacency(gg, sparse=FALSE, attr='V3')))
...
#   cc <- FFL.coherence(list(E_coli_mat2), cutoff.max = cutoff.max, cutoff.min = cutoff.min, target = which(colnames(E_coli_mat2)==gene))
#   return(cc)
# }, mc.cores = 100)
# write.csv(rbindlist(all_FFL), "scripts/data/all_genes_FFL.csv")
# 
