...
for (gen in seq(100, 5000, 100)) {
  ntw <- c(readRDS(paste0("scripts/data/Stab/list_plastic_topo_full_5000_",gen,".Rds")), readRDS(paste0("scripts/data/Stab/list_nnplast_topo_full_5000_",gen,".Rds")))
...
pdf("figures/supp/Stab_reg.pdf", width=10, height=4)
...
for (gen in seq(100, 5000, 100)) {
  df <- read.csv(paste0("scripts/data/Stab/full_5000_",gen,"_FFL.csv"))[1,]
  df$Gen <- gen
...
for (gen in seq(100, 5000, 100)) {
  df <- read.csv(paste0("scripts/data/Stab/full_5000_",gen,"_DMD.csv"))[1,]
  df$Gen <- gen
...
for (gen in seq(100, 5000, 100)) {
  df <- read.csv(paste0("scripts/data/Stab/full_5000_",gen,"_FFL.csv"))[2,]
  df$Gen <- gen
...
for (gen in seq(100, 5000, 100)) {
  df <- read.csv(paste0("scripts/data/Stab/full_5000_",gen,"_DMD.csv"))[2,]
  df$Gen <- gen
  df.gen.DMD2 <- rbind(df.gen.DMD2, df)  
}
plot(df.gen.DMD2$Gen, df.gen.DMD$FFL)

pdf("figures/supp/Stab_loop.pdf", width=8, height=5.5)
