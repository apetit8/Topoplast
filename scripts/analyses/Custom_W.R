...
#Network proprieties
Sensor <- 1 #Can't be regulated but can regulate TF
TF <- 4 #Regulated and regulate
Plastic <- 13 #Regulated and regulate TF
TG <- 13 #Regulated and regulate TF
Control <- 5 #Regulated
Total <- Sensor+TF+Plastic+TG+Control
##
W <- matrix(data=0, nrow=Total, ncol = Total)
cat(as.character(t(W)), file="templates/Matrix_W_supp.txt", sep=" ", append=FALSE)
#Columns
W[,c((Sensor):(Sensor+TF))] <- 1
#Rows
W[c((Sensor+1):(Sensor+TF)),c(1:Sensor)] <- 1
...
# cat(as.character(t(W)), file="../templates/Matrix_W.txt", sep=" ", append=FALSE)
W <- ifelse(W==0,"immut","normal")
cat(as.array(t(W)), file="templates/Matrix_W_supp.txt", sep=" ", append=TRUE)
