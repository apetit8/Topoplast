...
#Simulations
topo_plastic <- readRDS("scripts/data/list_plastic_topo_full_20k_0-01.Rds")
topo_nnplast <- readRDS("scripts/data/list_nnplast_topo_full_20k_0-01.Rds")
...
df <- data.frame(Pos=c(sum(reg_number_pos_np, na.rm=TRUE)*100/sum(reg_number_np, na.rm=TRUE),  sum(reg_number_pos_plast, na.rm=TRUE)*100/sum(reg_number_plast, na.rm=TRUE),  sum(THreg_sum_pos_np, na.rm=TRUE)*100/sum(THreg_sum_np, na.rm=TRUE), sum(THreg_sum_pos_plast, na.rm=TRUE)*100/sum(THreg_sum_plast, na.rm=TRUE) ),
                 Neg=c(sum(reg_number_neg_np, na.rm=TRUE)*100/sum(reg_number_np, na.rm=TRUE), sum(reg_number_neg_plast, na.rm=TRUE)*100/sum(reg_number_plast, na.rm=TRUE), sum(THreg_sum_neg_np, na.rm=TRUE)*100/sum(THreg_sum_np, na.rm=TRUE), sum(THreg_sum_neg_plast, na.rm=TRUE)*100/sum(THreg_sum_plast, na.rm=TRUE) ))
