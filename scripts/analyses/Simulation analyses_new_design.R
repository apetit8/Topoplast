...
#####################
sims.dirs1 <- list.dirs("simul/Full_netw_20k", recursive = TRUE)
genes <- 36
min <- 0.15
max <- 0.85
filename <- "full_20k_nnTFnnC"
################################################################################
...
plot(subset(df.full.gen, Gen >=10000)$Optimum, subset(df.full.gen, Gen >=10000)$Fitness)
dev.off()
