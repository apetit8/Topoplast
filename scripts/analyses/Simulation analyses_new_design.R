...
# Rprof()
topo_all <- mclapply(unique(df.last50[1:(nrow(df.last50)),1]), function(netw){ #CAREFULL only 1/5 simuls for test
  df <- subset(df.last50, data.dir==netw)
  topo <- lapply(1:genes, function(i){ #starts at 12 to not include TFs
    reg50 <- .lm.fit(cbind(rep(1, length(df$Pmean_1)), df$Pmean_1), df[,i+2])$coefficients # x9 times faster than lm()
    output <- essential.topo(df=subset(df.full, data.dir==netw), target=i,
...
################################################################################
############FFL
topo_plastic <- readRDS("scripts/data/list_plastic_topo_full_netw_drift.Rds")
topo_nnplast <- readRDS("scripts/data/list_nnplast_topo_full_netw_drift.Rds")
Plastic <- FFL.type2(topo_plastic, edges1 = 2, edges2 = 1, target= 2)
Nnplast <- FFL.type2(topo_nnplast, edges1 = 2, edges2 = 1, target= 2)
...
#####################
################################################################################
#CONTROL: REGULATIONS FROM RANDOM TF
################################################################################
#####################
sims.dirs1 <- list.dirs("simul/Full_netw", recursive = TRUE)
genes <- 36
...
for (i in 1:length(topo_plastic)) {
    topo_plastic[[i]][c(1, 3:11),2] <- sample(topo_plastic[[i]][c(1, 3:11),2])
    topo_plastic[[i]][2,c(1, 3:11)] <- sample(topo_plastic[[i]][2,c(1, 3:11)])
}  
...
for (i in 1:length(topo_nnplast)) {
    topo_nnplast[[i]][c(1, 3:11),2] <- sample(topo_nnplast[[i]][c(1, 3:11),2])
} 
