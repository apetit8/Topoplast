...
nr <- as.data.frame(cbind(nonreg_genes[,1],nonreg_genes[,1], rep(0, nrow(nonreg_genes)))) #Same file format as ec_cyc
...
phloops <- mclapply(phgenes, function(gene) {
  ...
  cc <- FBL.type(list(E_coli_mat3), cutoff = cutoff, target = which(colnames(E_coli_mat3)==gene))
  return(cc)
  print(gene)
}, mc.cores=22)
write.csv(rbindlist(phloops), "scripts/data/ph_plast_FBL.csv")
...
write.csv(rbindlist(plast_medgrowthloops), "scripts/data/plast_medium_growth_FBL.csv")
...
write.csv(rbindlist(np_medgrowthloops), "scripts/data/np_medium_growth_FBL.csv")
...
write.csv(rbindlist(plast_mg_c), "scripts/data/plast_mg_c_h_FBL.csv")

print("Carbone source done!")
########################################
...
write.csv(rbindlist(plast_ox), "scripts/data/plast_mg_c_FBL.csv")
...
write.csv(rbindlist(plast_aero), "scripts/data/plast_aero_FBL.csv")
...
write.csv(rbindlist(plast_juice), "scripts/data/plast_juice_FBL.csv")
...
write.csv(rbindlist(plast_temptr), "scripts/data/plast_temptr_FBL.csv")
...
write.csv(rbindlist(plast_stringent), "scripts/data/plast_stringent_FBL.csv")
...
write.csv(rbindlist(plast_stress), "scripts/data/plast_stress_FBL.csv")
...
plast_FBL <- mclapply(plast_genes, function(gene) {
  gg <- induced.subgraph(g, vids = c(which(colnames(E_coli_mat)==gene), which(colnames(E_coli_mat)%in%TF_genes)) )
  E_coli_mat2 <- t((get.adjacency(gg, sparse=FALSE, attr='V3')))
  ggg <- induced.subgraph(gg, vids = as.vector(unlist(neighborhood(gg, cutoff.max-1, nodes = which(colnames(E_coli_mat2)==gene), mode = 'all'))))
  E_coli_mat3 <- t((get.adjacency(ggg, sparse=FALSE, attr='V3')))
...
write.csv(rbindlist(plast_FBL), "scripts/data/plast_genes_FBL.csv")
...
nonplast_FBL <- mclapply(nonplast_genes, function(gene) {
  gg <- induced.subgraph(g, vids = c(which(colnames(E_coli_mat)==gene), which(colnames(E_coli_mat)%in%TF_genes)) )
  E_coli_mat2 <- t((get.adjacency(gg, sparse=FALSE, attr='V3')))
  ggg <- induced.subgraph(gg, vids = as.vector(unlist(neighborhood(gg, cutoff.max-1, nodes = which(colnames(E_coli_mat2)==gene), mode = 'all'))))
  E_coli_mat3 <- t((get.adjacency(ggg, sparse=FALSE, attr='V3')))
...
write.csv(rbindlist(nonplast_FBL), "scripts/data/nonplast_FBL.csv")

print("All non plastic genes done!")
# ########################################
# #Control : every genes
# 
# all_FBL <- mclapply(colnames(E_coli_mat), function(gene) {
#   cc <- FBL.type(list(E_coli_mat), cutoff = cutoff, target = which(colnames(E_coli_mat)==gene), randomFF=FALSE)
#   return(cc)
# }, mc.cores = 100)
# write.csv(rbindlist(all_FBL), "scripts/data/all_genes_FBL.csv")
# 
