...
rownames(df1) <- c("All","Non-plastic\ngenes", "Plastic\ngenes","Medgth", "pH1", "pH2","Aero","Mg_C","Stringent","T°1","T°2","Juice","Stress","Ox")
...
barplot(t(df1[,2:10]), col=c("grey","blue4","blue","dodgerblue3","skyblue3","lightskyblue","deepskyblue","cadetblue2","cyan3"),
        legend.text = c("No FFL",colnames(df1[,3:10])), args.legend = list(ncol=4, x = "topright", inset = c(0.3, 1.22)),
        main = "Feedforward motifs frequency")

barplot(t(df2[,2:12]), col=c("grey","#E65100","#FF6C00","#FB8C00","#FFB300","#FFCC80","#FDD835","#FFEE58","#FFF59D","#EEFF91","#FFFDE7"),
        legend.text = c("No Diamond",colnames(df2[,3:12])), args.legend = list(ncol=4, x = "topright", inset = c(0.27, 1.22)),
        main = "Diamond  motifs frequency")
dev.off()
