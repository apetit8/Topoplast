...
script.dir <- getwd()
user.dir  <- getwd()
param.dir <- file.path(script.dir, "templates/4g_connect")
ifelse(!dir.exists(file.path("simul/4g_connect")), dir.create("simul/4g_connect"), FALSE)#burning_andreas folder
cache.dir <- normalizePath(file.path(script.dir, "simul/4g_connect"))
...
#
launchfilename <- "launchers/4g_connect-launch.sh"
##########################
...
################################################################################
param.dir <- file.path(script.dir, "templates/3g_connect")
ifelse(!dir.exists(file.path("simul/3g_connect")), dir.create("simul/3g_connect"), FALSE)#burning_andreas folder
cache.dir <- normalizePath(file.path(script.dir, "simul/3g_connect"))
src.dir   <- file.path(script.dir, "functions")
...
max <- 0.85 #max environment
#
launchfilename <- "launchers/3g_connect-launch.sh"
##########################
# Generate all simulations
# A simulation is characterized by a name, a parameter file, and an extended parameter file with bottleneck etc. information
# Simulations involve the creation of many small files (new parameters every generation due to plasticity). This is largely
