...
#     optim <- make.randopt(runif(myparam$GENET_NBLOC, min, max), extparam$SCENARIO_PART1, sd=sd, min=min, max=max, random=random)
...
#           optim <- make.randopt(optim, extparam$SCENARIO_PART1, oldenv=optim[1], sd=sd, min=min, max=max, random=random) ## Here : give back the prev env.
...
#       optim <- make.randopt(optim, extparam$SCENARIO_PART2, begin.bottleneck=TRUE, sd=sd, min=min, max=max)
...
#           par.file.name <- c(par.file.name, file.path(repdir, .repFile(rep, gen)))
#           if (!file.exists(par.file.name[gen+1]) || overwrite) {
#             optim <- make.randopt(optim, extparam$SCENARIO_PART2, oldenv=optim[1], sd=sd, min=min, max=max, random=random)
#             write.param(par.file.name[gen+1],
#                         list(FITNESS_OPTIMUM = optim, 
#                              FILE_NEXTPAR    = suppressWarnings(normalizePath(file.path(repdir, .repFile(rep, gen+1))))))
#           }
...
#         par.file.name <- c(par.file.name, file.path(repdir, .repFile(rep, gen)))
#         if (!file.exists(par.file.name[gen + 1]) || overwrite) {
#           optim <- make.randopt(optim, extparam$SCENARIO_PART2, sd=sd, min=min, max=max) 
...
#     par.file.name <- c(par.file.name, file.path(repdir, .repFile(rep, gen)))
#     if (!file.exists(par.file.name[gen+1]) || overwrite) {
#       optim <- make.randopt(optim, extparlast, oldenv=optim[1], sd=sd, min=min, max=max, random=random)
#       write.param(par.file.name[gen+1],
#                   list(FITNESS_OPTIMUM = optim))
#     }
#     
